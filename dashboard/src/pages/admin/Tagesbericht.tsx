...
import { fetchDailyReport, exportDailyReport } from '../../services/reportApi';
...
const Tagesbericht: React.FC = () => {
  ...
  return (
    <Container>
      <Headline>Tagesbericht</Headline>
      <ExportRow>
        <Button onClick={() => exportDailyReport(date, groupId)}>Als CSV exportieren</Button>
      </ExportRow>
      <FilterRow>
        ...
        <tbody>
          {report.map(row => (
            <tr key={row.childId}>
              <td>{row.name}</td>
              <td>{row.checkIns}</td>
              <td>{row.checkedIn ? 'Ja' : 'Nein'}</td>
              <td>{row.checkedOut ? 'Ja' : 'Nein'}</td>
            </tr>
          ))}
        </tbody>
      </table>
